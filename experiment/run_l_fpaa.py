...
from experiment.run_strat_search import derive_l_star, derive_m_star
...
def main(pgm: cirq.Circuit, w: float, confidence: float):  # WLOG we just assume input sv is always zeros
    ...
    print(f"Derived l ={l_star} and {m_star}")
    ...
    strat = HypeParamConfig(DELTA=DELTA, strat=[(l_star, m_star)], NUM_base_execution=None, label=None)
    print("Found Strat is")
    print(str(strat))
    assert_check_fpaa(expect_sv=None, pgm=pgm, config=strat, logging_option=logging.INFO)
